...
def get_soft_scores_and_true_labels(dataset, model):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            pred = model(inputs)
            all_first_soft_scores += (pred[:,0].tolist())
            all_second_soft_scores += (pred[:,1].tolist())
            gt_labels += targets
...
def main():
    ...
    # tal flag delete me start ############################
    # args.model ="SimpleNet"
    # args.checkpoint_path = "checkpoints/fakes_dataset_SimpleNet_Adam.pt"
    # args.dataset = "fakes_dataset"
    # tal flag delete me end ############################
