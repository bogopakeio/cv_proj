...
class Xception(nn.Module):
    ...
    def forward(self, x): #x shape 32,3,256,256
        x = self.conv1(x) # -> 32,32,127,127
        x = self.bn1(x)
        x = self.relu(x)

        x = self.conv2(x) #-> 32,64,125,125
        x = self.bn2(x)
        x = self.relu(x)

        x = self.block1(x)  # -> 32,128,63,63
        x = self.block2(x)  # -> 32,256,32,32
        x = self.block3(x)  # -> 32,728,16,16
        x = self.block4(x)  # -> 32,728,16,16
        x = self.block5(x)  # -> 32,728,16,16
        x = self.block6(x)
        x = self.block7(x)
        x = self.block8(x)
        x = self.block9(x)
        x = self.block10(x)
        x = self.block11(x) # -> 32,728,16,16
        x = self.block12(x) # -> 32,1024,8,8

        x = self.conv3(x)   # -> 32,1536,8,8
        x = self.bn3(x)
        x = self.relu(x)

        x = self.conv4(x)   # -> 32,2048,8,8
        x = self.bn4(x)
        x = self.relu(x)

        x = F.adaptive_avg_pool2d(x, (1, 1)) # -> 32,2048,1,1
